...
(by prior work of the programme) is identified as $\zetaK(s)\zetaK(s-1)$ by the classical
Siegel--Weil / maximal-order Eisenstein framework and checked by complete short-vector
enumeration of the local representation data. (The theorem carries the identity; the
computation verifies the implementation, not an infinite Euler product by itself.) We then map the \emph{boundary} of this shadow: using a reproducible
...
\emph{exactly} the rank-two product $\zetaK(s)\zetaK(s-1)$ --- an identity carried by the
classical Siegel--Weil/maximal-order theorem (Proposition~\ref{prop:icosian}) and checked
by complete short-vector enumeration in the maximal icosian order in \cite{IcosianTriad}
(the theorem supplies the identity; the computation verifies the implementation, not an
infinite Euler product by itself).
...
series has critical-line zeros matching those of $\zeta(s)\zeta(s-1)$ (tested to finite
...
The $96$-vertex $\varphi$-shell (Galois-sign component) has tested critical-line zeros matching
...
The achievement of this section is \emph{not} that the object contains $\zeta$---many
constructions do, and that alone is not special. It is the pair: the object carries a
precise Dedekind/Eisenstein arithmetic shadow, \emph{and} the stronger
``object $=\zeta$'' identity provably fails \emph{as an identity of $L$-functions}. The
object's $L$-function
(Proposition~\ref{prop:icosian}) has degree $4$ and, by \eqref{eq:dedekind}, equals
$\zeta(s)L(s,\chi_5)\zeta(s-1)L(s-1,\chi_5)$, with no local-$2$ correction; the unshifted
$\zeta(s)$ is one factor among four (the shifted $\zeta(s-1)$ is also present but is
irrelevant to the critical-line count, by Proposition~\ref{prop:online}). Whether the
object \emph{isolates} $\zeta$ is the real question; it does not, and we certify this.

\paragraph{The gate.} A proposed correspondence supplies a parameter-free procedure
producing the critical-line zeros of a claimed $L$-function \emph{without taking the target as
input}. The gate checks \textbf{fingerprint} (nearest-neighbour spacing: fraction of
normalized gaps below $0.3$), \textbf{pointwise} (first eight zeros, tolerance $0.2$),
\textbf{density} (critical-line zeros per unit height), \textbf{rigidity} (number variance),
structural \textbf{degree}/\textbf{pole}, and---auxiliary only---\textbf{no-hardcoding}
/ \textbf{provenance} (declared-procedure checks that catch a \emph{declared} fitted map,
relying on honest declaration, not program analysis). \textbf{The decisive rejections
below are structural} (degree, density, pointwise); provenance is a guardrail, not the
proof. A hashed certificate is emitted either way; the engine is a verification certifier, not a
search. The geometry$\to$arithmetic enumeration itself is the separate computation of
\cite{IcosianTriad}, not re-performed by the gate.

\begin{result}[the $\zeta$-boundary]\label{res:o2}
The strong candidate ``object $\leftrightarrow\zeta$'' is \textbf{rejected}. Against
$\zeta$ (degree $1$) the object's degree-$4$ critical-line content fails: \emph{degree}
...
critical-line lists, the object's combined zero count exceeds $\zeta$'s---the count ratios are
...
theorem.) A list denser than $\zeta$'s on these windows is not $\zeta$'s list in these
certified windows.
\end{result}
...
fitted-map trap of a prior internal audit (the ``$W5$'' provenance audit of the
substrate$\to$prime map).
\end{result}
...
\hline
$24$-cell $\to \zeta(s)\zeta(s-1)$ (critical-line) & \textsc{consistent} & --- \\
icosian $\to \zetaK(s)\zetaK(s-1)$ (exact) & \textsc{verified} & --- \\
$\varphi$-shell $\to L(s,\chi_5)$ (critical-line) & \textsc{consistent} & --- \\
...
\S\ref{sec:shadow} (with no local-$2$ correction; including the inert-prime-$2$ check),
and the certified boundary of
...
paper in \texttt{engine/} (the seeded GUE control row is produced by
