...
\newcommand{\Wtrunc}{W_{\le 150}}

\title{The Prime Side of a Weil Witness from the Icosian Triad:\\ the Gap, Localized}
...
\begin{center}
\fbox{\parbox{0.93\linewidth}{\small\textbf{Read first.} The witness here is \textbf{not}
a witness for the classical Riemann zeta function. It is a Weil witness for a
\emph{specific cuspidal level-$31$ $L$-function} attached to the object's Brandt face;
classical $\zeta$ sits in a separate (Eisenstein) face. Its universal positivity
quantifier is therefore \textbf{GRH for that cuspidal $L$-function, not classical RH},
and its relevance to RH is \textbf{structural, not a reduction}. We supply the
parameter-free \emph{prime side}; the archimedean side is the standard completion of the
cuspidal $L$-function. No claim about RH is made.}}
\end{center}
\medskip
...
on $\zeta$ (where the zeros are known) and, on a finite Gaussian test family, returns
nonnegative values of the truncated computed functional $\Wtrunc(h)$ (prime side cut at
$N(\fq)\le150$). \textbf{We make no claim about the Riemann Hypothesis.}
Crucially: the
...
This face \emph{contains} classical $\zeta$. Its parameters are $a_\fq=N(\fq)+1$, which
...
read directly from the icosian Brandt matrices---no point-counting, no fitting. (In the
external route-B computation \cite{BrandtCuspidal} these eigenvalues were further checked
out-of-sample against point counts; that comparison lives in that bundle, not this one.)
The computed $a_\fq$ obey the Ramanujan bound in the tested range;
...
$\Lambda(s)=N^{s/2}L_\infty(s)L(s)$ (gamma factor $L_\infty$ and conductor
$N=31\cdot25=775$). For a test
...
\]
the sum running over the nontrivial zeros $\rho$ of $L(s)$, \emph{with no assumption on
$\mathrm{Re}\,\rho$} \cite{Weil}. Writing $\rho=\tfrac12+i\gamma$ with $\gamma\in\R$ is
precisely GRH for this $L$, so we do not presuppose it. The
...
For the cuspidal level-$31$ $L$-function, the truncated computed functional
$\Wtrunc(h)\ge 0$ (prime side cut at $N(\fq)\le150$, no tail bound) on every test in the
family ($\sigma_h=3,4,5,6$; $\Wtrunc=+1.4047,+3.6961,+6.3931,+9.4125$). This is the
implementation's value on a finite family, not the full Weil functional $W(h)$.
\end{result}
...
Eisenstein input is not admissible for this cuspidal Satake residual---without implying
the Eisenstein face is disqualified from all explicit-formula positivity.
\end{result}
...
prime term is truncated to the cached range $N(\fq)\le150$ with no tail bound.
\textbf{The numerical tests validate normalization and implementation; they are not
evidence for the universal positivity quantifier.} $W(h)\ge0$ here is consistent with
GRH for this $L$-function but is not a sharp test of it; we supply no sharp test family.
\end{remark}

\begin{remark}[what a sharp test would require---future work]\label{rem:future}
A genuine test of the quantifier needs: (i) \emph{adversarial} positive-type test
functions $h$ for which $\mathrm{PRIME}(h)$ is comparable to $\mathrm{ARCH}(h)$ (so the
prime side can actually threaten positivity), and (ii) a certified \emph{tail bound} on
the truncated prime residual beyond $N(\fq)\le150$. Neither is supplied here; both are
the next required steps before any positivity statement carries weight.
\end{remark}
...
form's completion. The implementation is validated on $\zeta$ and gives
$\Wtrunc(h)\ge0$ (the truncated functional, $N(\fq)\le150$) on
...
\bibitem{BrandtCuspidal} VFD programme, route-B icosian Brandt computation (external
  \texttt{icosian\_brandt\_cuspidal\_geometry/route\_b} source; self-contained copies
  \texttt{weil\_wall.py}, \texttt{geometric\_aP.py} ship in this bundle's
  \texttt{repro/src}): geometric Hecke eigenvalues $a_\fq$ of the self-adjoint
